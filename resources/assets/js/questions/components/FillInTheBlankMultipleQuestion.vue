    ...
    <div>
        ...
        <div class="checkbox">
            <label for="is_fractional" class="col-sm-offset-2">
                <input type="checkbox" id="is_fractional" name="is_fractional" value="1" v-model="is_fractional"> Is Fractional
            </label>
        </div>

        <div class="checkbox">
            <label for="is_ordered" class="col-sm-offset-2">
                <input type="checkbox" id="is_ordered" name="is_ordered" value="1" v-model="is_ordered"> Is Ordered
            </label>
        </div>
    ...
    export default{
      ...
      methods   : {
        ...
        _validateInputs(){
          var messages = [],
              check    = true,
              answers  = _.filter(this.answers, answer => answer.active == true);


          // All choice content must has something
          if (_.filter(answers, answer => {
        ...
        _choiceQuestionTypeId(){
          return _.find(this.questionTypes, {"code": "MultipleFillInBlanks"}).id
        },
      }
    }
</script>
