		...
		<div class="panel panel-default">
			...
			<div class="panel-footer clearfix">
				@if($lesson->hasTest())
					...
					@else
						<a class="btn btn-primary pull-right" href="{{route('show.lesson.test', $lesson->id)}}">{{__("Test")}}</a>
					@endif
				@endif
				<button class="btn btn-info pull-right" @click="back" style="margin-right:15px">{{__("Back")}}</button>
			</div>
	    </div>
	</div>
@endsection
